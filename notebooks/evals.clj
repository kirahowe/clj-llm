...
;; This is the chapter the library is named for. An assay is a quantitative test of quality, and the premise here is simple: there is no iterating toward better models, prompts, parameters, or pipelines without measuring, so measurement should not be an optional add-on with its own framework to adopt — it should fall out of the calls you are already making.

;; assay's eval system is two layers. The first layer is passive: every response is already a measurement. The second is active: suites run cases against variants and score the results. Between them sits the workflow this chapter builds up to — real traffic becomes cases, cases become scored comparisons, comparisons gate changes.
...
(select-keys (assay/generate config "What is the capital of France?")
             [:assay/request :assay/usage :assay/latency-ms :assay/started-at :assay/op])

;; A response with these keys is called an *interaction record*. To collect records from live traffic, set `:assay/on-interaction` — a function of one record — once in your config's defaults. Here we collect into an atom; in production this is typically an append to a log, a queue, or a table:

(def interactions (atom []))

(def traffic-config
  (assoc-in config [:assay/defaults :assay/on-interaction]
            (fn [record] (swap! interactions conj record))))

(run! #(assay/generate traffic-config %)
...
;; The report is data all the way down. `:assay/results` holds one entry per case×variant with the full response and its scores; `:assay/summary` aggregates per variant; and the report records its own provenance — when it ran and how big it was — so a stored report is meaningfully comparable with next month's:

(select-keys report [:assay/run-at :assay/case-count :assay/variant-count])

(first (:assay/results report))

;; A **variant** is just a bundle of request keys — model, system prompt, temperature, tools, anything `generate` accepts — so "compare two models", "compare two prompts" and "compare with/without tools" are all the same operation. Cases accept either `:assay/input` (a prompt) or `:assay/messages` (a full conversation), which is exactly what makes collected records replayable:

(let [record (first @interactions)]
  (-> (eval/run config
                #:assay{:cases [#:assay{:id :replayed
                                        :messages (:assay/messages (:assay/request record))
                                        :expected "Paris"}]
                        :scorers [:includes]})
      :assay/summary))
...
;; Three built-ins cover mechanical ground truth: `:exact-match` (trimmed equality with `:assay/expected`), `:includes` (case-insensitive containment), and `:matches` (regex). A custom scorer is any function of the context map `{:config _ :case _ :variant _ :response _}` returning `{:score <0.0-1.0>}` plus anything else worth keeping. Unqualified keys on a case are yours, so scorers can read custom fields:

(defn terse-enough?
  "Full marks under 60 characters, scaled down to zero at 300."
  [{:keys [response]}]
  (let [n (count (str (:assay/text response)))]
    {:score (max 0.0 (min 1.0 (/ (- 300.0 n) 240.0)))
     :length n}))

(-> (eval/run config (assoc suite :assay/scorers [:includes terse-enough?]))
    :assay/summary)
...
;; By default, a case×variant runs a single `generate` call. But the question you actually care about is usually one level up: is my *pipeline* good — retrieval plus prompt assembly plus the model plus post-processing? Point `:assay/task` at any function of `{:keys [config case variant]}` that returns a response-shaped map, and the same cases, scorers, thresholds and reports apply to the whole system:
...
(defn faq-pipeline
  ...
  (let [doc "Support doc: to reset a password, click 'Forgot password' and follow the emailed reset link."]
    (assay/generate config
                    #:assay{:system (str "Answer using this document:\n" doc)
                            :prompt (:assay/input case)})))

(-> (eval/run config
              #:assay{:cases [#:assay{:id :password-reset
...
;; The task contract is deliberately thin: return at least `:assay/text` (or whatever your scorers read); return real `generate` responses — as `faq-pipeline` does — and latency and token summaries stay accurate for free. Variants still work with a custom task: the variant map is passed to your task, so variants can select pipeline configurations, not just request keys. In EDN suites, `:assay/task` can be a qualified symbol.
...
;; A report you have to eyeball is a report that stops being read. `:assay/thresholds` sets a minimum mean score per scorer; the report then carries `:assay/passed?`, and the CLI (`bb eval`, or `clojure -M:dev -m assay.eval`) exits non-zero when a threshold is missed or any case errors — so a suite drops into CI like any other test suite:

(let [gated (assoc suite :assay/thresholds {:includes 0.9})]
  (select-keys (eval/run config gated) [:assay/passed? :assay/thresholds]))
...
;; 1. Ship with `:assay/on-interaction` collecting records from day one — it's one line of config.
...
;; 5. Set `:assay/thresholds` and wire `bb eval` into CI, so quality regressions fail builds the way broken tests do.
