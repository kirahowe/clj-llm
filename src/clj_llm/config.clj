(ns clj-llm.config
  ...
  get aero's full tag set (#env, #or, #profile, #include, #ref, ...):
    ...
    #:clj-llm{:providers
            ...
            :models {:smart #:clj-llm{:provider :anthropic
  ...
  account, a local Ollama server). The :clj-llm/adapter key selects the
  ...
  :smart and the vendor mapping lives in config. :clj-llm/defaults are
  merged into every request; that includes :clj-llm/on-interaction, a hook
...
(defn provider-name
  "The name a provider config was registered under in :clj-llm/providers."
  [provider-config]
...
(defn provider-config
  ...
  (if-let [p (get-in config [:clj-llm/providers provider-name])]
    (assoc p ::name provider-name)
    (throw (ex-info (str "No provider named " provider-name " in config. "
                         "Known providers: "
                         (pr-str (keys (:clj-llm/providers config))))
                    {:type :clj-llm/config-error
                     :provider provider-name
                     :known (keys (:clj-llm/providers config))}))))

(defn resolve-model
  "Resolve a model designator into {:provider <provider-config> :model <string>}.

  Designators:
    nil       use the default alias from [:clj-llm/defaults default-key]
    keyword   an alias defined under :clj-llm/models
    string    \"provider-name/model-id\" — splits on the first slash
    map       #:clj-llm{:provider <name> :model \"model-id\"} used directly

  default-key is :clj-llm/model or :clj-llm/embedding-model (defaults to
  :clj-llm/model)."
  ([config designator] (resolve-model config designator :clj-llm/model))
  ([config designator default-key]
   (cond
     (nil? designator)
     (if-let [d (get-in config [:clj-llm/defaults default-key])]
       (resolve-model config d default-key)
       (throw (ex-info (str "No model given and no " default-key
                            " configured under :clj-llm/defaults")
                       {:type :clj-llm/config-error :default-key default-key})))

     (keyword? designator)
     (if-let [alias-config (get-in config [:clj-llm/models designator])]
       (resolve-model config alias-config default-key)
       (throw (ex-info (str "No model alias " designator " in config. "
                            "Known aliases: "
                            (pr-str (keys (:clj-llm/models config))))
                       {:type :clj-llm/config-error
                        :alias designator
                        :known (keys (:clj-llm/models config))})))
     ...
     (let [i (str/index-of designator "/")]
       (when-not i
         (throw (ex-info (str "String model designators must look like "
                         ...
                         {:type :clj-llm/config-error :designator designator})))
       {:provider (provider-config config (keyword (subs designator 0 i)))
        :model (subs designator (inc i))})

     (map? designator)
     {:provider (provider-config config (:clj-llm/provider designator))
      :model (:clj-llm/model designator)}
